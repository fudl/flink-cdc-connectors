...
public class SqlServerConnection extends JdbcConnection {
    ...
    private static final String URL_PATTERN =
            "jdbc:sqlserver://${" + JdbcConfiguration.HOSTNAME + "}";
    ...
    private static final Field AGENT_STATUS_QUERY =
            Field.create("sqlserver.agent.status.query")
    ...
    public SqlServerConnection(
            SqlServerJdbcConfiguration config,
        ...
        super(
                config,
                createConnectionFactory(config, useSingleDatabase),
    ...
    public SqlServerConnection(
        ...
        this(config, sourceTimestampMode, valueConverters, skippedOperations, useSingleDatabase);
    ...
    private static ConnectionFactory createConnectionFactory(
        ...
        return JdbcConnection.patternBasedFactory(
                createUrlPattern(config, useSingleDatabase),
     ...
     */
    public boolean checkIfConnectedUserHasAccessToCDCTable(String databaseName)
            throws SQLException {
        final AtomicBoolean userHasAccess = new AtomicBoolean();
        final String query =
                replaceDatabaseNamePlaceholder(
                        "EXEC [#db].sys.sp_cdc_help_change_data_capture", databaseName);
    ...
    public boolean isAgentRunning(String databaseName) throws SQLException {
        final String query =
                replaceDatabaseNamePlaceholder(
                        config().getString(AGENT_STATUS_QUERY), databaseName);
