...
public class SqlServerConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    public static enum SnapshotMode implements EnumeratedValue {
        ...
        /**
         * Perform a snapshot of data and schema upon initial startup of a connector but does not
        ...
        /**
         * Whether this snapshotting mode should include the actual data or just the schema of
         * captured tables.
         */
    ...
    public static enum SnapshotIsolationMode implements EnumeratedValue {
         ...
         *
         * <p>The connector will execute {@code SELECT * FROM .. WITH (TABLOCKX)}
         */
        EXCLUSIVE("exclusive"),

        /**
         * This mode uses SNAPSHOT isolation level. This way reads and writes are not blocked for
         * the entire duration of the snapshot. Snapshot consistency is guaranteed as long as DDL
         * statements are not executed at the time.
         */
        SNAPSHOT("snapshot"),
        ...
        /**
         * This mode uses READ COMMITTED isolation level. This mode does not take any table locks
         * during the snapshot process. In addition, it does not take any long-lasting row-level
         * locks, like in repeatable read isolation level. Snapshot consistency is not guaranteed.
         */
        READ_COMMITTED("read_committed"),

        /**
         * This mode uses READ UNCOMMITTED isolation level. This mode takes neither table locks nor
         * row-level locks during the snapshot process. This way other transactions are not affected
         * by initial snapshot process. However, snapshot consistency is not guaranteed.
         */
    ...
    public static final Field USER =
            RelationalDatabaseConnectorConfig.USER.optional().withNoValidation();
    ...
    public static final Field INSTANCE =
            Field.create(DATABASE_CONFIG_PREFIX + SqlServerConnection.INSTANCE_NAME)
    ...
    public static final Field DATABASE_NAMES =
            Field.create(DATABASE_CONFIG_PREFIX + "names")
    ...
    public static final Field MAX_LSN_OPTIMIZATION =
            Field.createInternal("streaming.lsn.optimization")
                    .withDisplayName("Max LSN Optimization")
                    .withDefault(true)
                    .withType(Type.BOOLEAN)
                    .withImportance(Importance.LOW)
                    .withDescription(
                            "This property can be used to enable/disable an optimization that prevents querying the cdc tables on LSNs not correlated to changes.");

    public static final Field MAX_TRANSACTIONS_PER_ITERATION =
            Field.create(MAX_TRANSACTIONS_PER_ITERATION_CONFIG_NAME)
    ...
    public static final Field SOURCE_TIMESTAMP_MODE =
            Field.create(SOURCE_TIMESTAMP_MODE_CONFIG_NAME)
                    ...
                    .withImportance(Importance.LOW)
                    .withDescription(
                            "Configures the criteria of the attached timestamp within the source record (ts_ms)."
                                    + "Options include:"
                                    + "'"
    ...
    public static final Field SNAPSHOT_ISOLATION_MODE =
            Field.create("snapshot.isolation.mode")
                    ...
                    .withImportance(Importance.LOW)
                    .withDescription(
                            "Controls which transaction isolation level is used and how long the connector locks the captured tables. "
                                    + "The default is '"
                                    ...
                                    + "Using a value of '"
                                    + SnapshotIsolationMode.EXCLUSIVE.getValue()
                                    ...
                                    + "When '"
                                    + SnapshotIsolationMode.SNAPSHOT.getValue()
    ...
    public static final Field INCREMENTAL_SNAPSHOT_OPTION_RECOMPILE =
            Field.create("incremental.snapshot.option.recompile")
                    ...
                    .withDescription(
                            "Add OPTION(RECOMPILE) on each SELECT statement during the incremental snapshot process. "
    ...
    private static final ConfigDefinition CONFIG_DEFINITION =
            HistorizedRelationalDatabaseConnectorConfig.CONFIG_DEFINITION
                    .edit()
                    .name("SQL Server")
                    .type(DATABASE_NAMES, HOSTNAME, PORT, USER, PASSWORD, INSTANCE)
                    .connector(
                            SNAPSHOT_MODE,
    ...
    public SqlServerConnectorConfig(Configuration config) {
        ...
        this.readOnlyDatabaseConnection =
                READ_ONLY_INTENT.equals(config.getString(APPLICATION_INTENT_KEY));
        if (readOnlyDatabaseConnection) {
            this.snapshotIsolationMode = SnapshotIsolationMode.SNAPSHOT;
            LOGGER.info(
                    "JDBC connection has set applicationIntent = ReadOnly, switching snapshot isolation mode to {}",
                    SnapshotIsolationMode.SNAPSHOT.name());
        } else {
        ...
        this.sourceTimestampMode =
                SourceTimestampMode.fromMode(config.getString(SOURCE_TIMESTAMP_MODE_CONFIG_NAME));
        ...
        if (!config.getBoolean(MAX_LSN_OPTIMIZATION)) {
            LOGGER.warn(
                    "The option '{}' is no longer taken into account. The optimization is always enabled.",
                    MAX_LSN_OPTIMIZATION.name());
        }
    ...
    public SqlServerJdbcConfiguration getJdbcConfig() {
        ...
        if (useSingleDatabase()) {
            config = JdbcConfiguration.copy(config).withDatabase(databaseNames.get(0)).build();
        }
        SqlServerJdbcConfiguration sqlServerconfig = SqlServerJdbcConfiguration.adapt(config);
        if (getInstanceName() != null) {
            sqlServerconfig =
                    SqlServerJdbcConfiguration.copy(config).withInstance(getInstanceName()).build();
        }
    ...
    private static class SystemTablesPredicate implements TableFilter {
        ...
        public boolean isIncluded(TableId t) {
            return t.schema() != null
                    && !(t.schema().toLowerCase().equals("cdc")
        ...
        return new HistoryRecordComparator() {
            ...
            protected boolean isPositionAtOrBefore(Document recorded, Document desired) {
                return Lsn.valueOf(recorded.getString(SourceInfo.CHANGE_LSN_KEY))
                                .compareTo(
                                        Lsn.valueOf(desired.getString(SourceInfo.CHANGE_LSN_KEY)))
                        < 1;
            }
        };
    }
    ...
    // getConfig().getTrimmedStrings(SNAPSHOT_SELECT_STATEMENT_OVERRIDES_BY_TABLE, ",");
    //
    // 	if (tableValues == null) {
    ...
    private static int validateDatabaseNames(
