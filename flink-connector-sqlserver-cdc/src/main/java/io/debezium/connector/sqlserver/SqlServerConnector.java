...
public class SqlServerConnector extends RelationalBaseSourceConnector {
    ...
    public List<Map<String, String>> taskConfigs(int maxTasks) {
        if (maxTasks > 1 && !properties.containsKey(DATABASE_NAMES.name())) {
            throw new IllegalArgumentException(
                    "Only a single connector task may be started in single-partition mode");
        }

        final SqlServerConnectorConfig config =
                new SqlServerConnectorConfig(Configuration.from(properties));
    ...
    private List<Map<String, String>> buildTaskConfigs(
            SqlServerConnection connection, SqlServerConnectorConfig config, int maxTasks) {
        ...
        for (int databaseNameIndex = 0;
                databaseNameIndex < databaseNames.size();
                databaseNameIndex++) {
            int taskIndex = databaseNameIndex % maxTasks;
            String realDatabaseName =
                    connection.retrieveRealDatabaseName(databaseNames.get(databaseNameIndex));
    ...
    protected void validateConnection(Map<String, ConfigValue> configValues, Configuration config) {
        ...
        final ConfigValue hostnameValue =
                configValues.get(RelationalDatabaseConnectorConfig.HOSTNAME.name());
        final ConfigValue userValue =
                configValues.get(RelationalDatabaseConnectorConfig.USER.name());
        ...
        try (SqlServerConnection connection = connect(sqlServerConfig)) {
            ...
            if (sqlServerConfig.getSnapshotMode()
                    != SqlServerConnectorConfig.SnapshotMode.INITIAL_ONLY) {
                ...
                if (!noAccessDatabaseNames.isEmpty()) {
                    String errorMessage =
                            String.format(
                                    ...
                                    config.getString(RelationalDatabaseConnectorConfig.USER),
                                    String.join(", ", noAccessDatabaseNames));
            ...
            LOGGER.error(
                    ...
                    config.withMaskedPasswords(),
                    userValue,
                    e);
            hostnameValue.addErrorMessage(
                    "Unable to connect. Check this and other connection properties. Error: "
                            + e.getMessage());
        }
    }
